...
#if 1
    #define err(str, ...)       do { utils_printf("[%s:%u] " str, __func__, __LINE__, ##__VA_ARGS__); while(1); } while(0)
#else
    #define err(str, ...)
#endif
//=============================================================================
...
#if defined(CONFIG_ENABLE_SHELL_CMD_HISTORY)
    char            *pHistory_buf;
    uint16_t        line_size;
    uint16_t        cmd_deep;
    char            **ppLine_list;
...
_shell_log(const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    utils_vsprintf(log_buf, sizeof(log_buf), fmt, args);
    va_end(args);
...
_shell_push_to_history(char *pLine)
{
    ...
    do {
        ...
        utils_snprintf(pAct_line, g_sh_dev.line_size, "%s", pLine);
    ...
    int (*cb_write)(uint8_t*, uint32_t, void*))
{
    ...
    do {
        ...
        {
            char    str_buf[12] = {0};
            utils_snprintf(str_buf, 12, "\x1b[%dC", (int)(wr_pos - cursor_pos));
    ...
    int (*cb_write)(uint8_t*, uint32_t, void*))
{
    ...
    switch( esc_code )
    {
        ...
        case SH_ESC_CODE_HOME:
            ...
            {
                char    str_buf[12] = {0};
                utils_snprintf(str_buf, 12, "\x1b[%dD", (int)(*pCursor_pos));
                cb_write((uint8_t*)str_buf, strlen(str_buf), g_sh_dev.pUser_data);
                *pCursor_pos = 0;
            }
            break;
        case SH_ESC_CODE_END:
            ...
            {
                char    str_buf[12] = {0};
                utils_snprintf(str_buf, 12, "\x1b[%dC", (int)((*pWr_pos) - (*pCursor_pos)));
                cb_write((uint8_t*)str_buf, strlen(str_buf), g_sh_dev.pUser_data);
                *pCursor_pos = *pWr_pos;
            }
            break;
...
//                }
            ...
            {   // non-escape text
                ...
                {
                    char        str_buf[12] = {0};
                    utils_snprintf(str_buf, 12, "\x1b[%dD", (int)(g_sh_dev.wr_pos - g_sh_dev.cursor_pos - 1));
                    cb_write((uint8_t*)str_buf, strlen(str_buf), g_sh_dev.pUser_data);
                }
...
_sh_cmd_help(int argc, char **argv, cb_shell_out_t log_out, void *pExtra)
{
    ...
    if (argc == 2)
    {
        pCur = (sh_cmd_t*)&__cli_cmd_pool_start__;
        while( (uintptr_t)pCur < (uintptr_t)&__cli_cmd_pool_end__  )
        {
...
const sh_cmd_t   g_sh_cmd_desc_help __clicmd_pool =
{
    .pCmd_name    = "help",
        ...
        if( pIO )
        {
            ...
            if( rval == SH_STATE_GET_LINE )
            {
                ...
                if( arg_cnt )
                {
                    ...
                    pCmd_cur = (sh_cmd_t*)&__cli_cmd_pool_start__;
                    while( (uintptr_t)pCmd_cur < (uintptr_t)&__cli_cmd_pool_end__  )
                    {
